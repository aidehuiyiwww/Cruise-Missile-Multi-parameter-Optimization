...
epsilon=1;   %收敛精度
k=15;    %随机搜索方向个数
a=[150;10;0.1;0.1;400];   %自变量下限值
b=[190;16;0.9;0.9;500];    %自变量上限值
dx=[50;6;1;1;200];
%%
...
while 1
    for i=1:1:n
        x0(i)=a(i)+(b(i)-a(i))*rand(1,1);  %产生一个随机初始点，由约束1得自变量取值区间为（-3，3）
    end
    if x0(3)<x0(4)
        continue
    end
    g=gcon(x0)   %计算产生的随机点约束函数值
    if res_compare(g)<0.1
        break;
    end
end
x00=x0;
...
while 1
    ...
    while 1
        i=1;    %用来记录沿随机方向移动后可行点数目的计数变量
        while i<(k+1)
          r(:,i)=rands(n,1);    %产生一个坐标（-1，1）之间的随机方向
          e(:,i)=r(:,i)/norm(r(:,i));   %单位化随机方向
          xs=x0+a0*e(:,i).*dx;  %计算沿随机方向移动后的自变量点
          gz=gcon(xs);          
          if res_compare(gz)<0.1
            fs=fun(xs); %计算移动后可行点的目标函数值
            x(:,i)=xs;  %记录沿随机方向移动后的可行点
            fz(:,i)=fs; %记录沿随机方向移动后的可行点目标函数值
            i=i+1;
          else
             continue;
          end
...
for i=2:k-1 
    plot([mm(i-1) mm(i)],[fc(i-1) fc(i)],'b*-'); 
    xlabel("迭代次数");
    ylabel("函数值");
    hold on; 
end 
str=strcat("函数值：",num2str(f0,"%.1f"));
title(str);
xe=x0;
ge=gcon(x0);
fe=f0;
xe,fe,ge  %输出最优点坐标及函数值
toc  %计时结束
